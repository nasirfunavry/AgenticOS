...
// Get salt and IV from environment variables
 ...
 */
export async function loadTokens(encryptionKey: string): Promise<TwitterTokens> {
    ...
    const accessToken = await decrypt(encryptedTokens.encryptedAccessToken, encryptionKey);
    const refreshToken = await decrypt(encryptedTokens.encryptedRefreshToken, encryptionKey);
